...
const SignUp = () => {
    ...
    return (
        <div className="sign_container">
            ...
            <input autoSave="off" onChange={e => setUsername(e.target.value)} className={`sign_input ${usernameInputStatus ? 'error' : ''}`} type="text" placeholder="نام کاربری" />
            <input autoSave="off" onChange={e => setPassword(e.target.value)} className={`sign_input ${passwordInputStatus ? 'error' : ''}`} type="password" placeholder="رمز عبور" />
            <input autoSave="off" className="sign_input" type="email" placeholder="ایمیل ( اختیاری )" />
